...
export const generateArchitecturalVision = (state: any): string => {
  try {
    ...
    const purposeSpaces = phase3?.purposeSpaces || {};
    const spaces = Object.keys(typeof purposeSpaces === 'object' && purposeSpaces !== null ? purposeSpaces : {}).join(', ') || (isEmpresa ? "áreas estratégicas" : "diversos espacios");
    
    const narratedText = String(phase6?.narratedText || "");
    const emotion = phase3?.mainEmotion || (isEmpresa ? "propósito" : "armonía");
    
    const selectedWords = phase1?.selectedWords;
    const words = Array.isArray(selectedWords) ? selectedWords.join(', ') : (isEmpresa ? "identidad" : "equilibrio");
    ...
    console.error("Error generating architectural vision:", err);
    return "Tras procesar tu viaje emocional, hemos diseñado una propuesta especial para ti basada en el equilibrio y la armonía, donde cada espacio es un reflejo de tu esencia.";
  }
}
